def generate_feedback(score_result: dict):
    ...
    if total_score >= 80:
        feedback.append("전체 발표 자세, 시선, 말하기 흐름이 안정적입니다.")
    elif total_score >= 60:
        feedback.append("전체적으로 무난하지만 자세, 시선, 말하기 흐름 중 일부 개선이 필요합니다.")
    else:
        feedback.append("발표 자세, 시선 처리, 말하기 흐름 전반의 개선이 필요합니다.")

    if pose_rate < 70:
        feedback.append("영상에서 발표자의 전신 또는 상반신이 안정적으로 인식되지 않았습니다.")

    if face_rate < 70:
        feedback.append("얼굴 인식률이 낮아 시선 분석 정확도가 떨어질 수 있습니다.")

    if shoulder_score < 70:
        feedback.append("어깨 균형이 일정하지 않아 자세가 흔들려 보일 수 있습니다.")

    if gaze_score < 70:
        feedback.append("정면 응시가 부족하거나 고개가 한쪽으로 돌아간 구간이 있습니다.")
