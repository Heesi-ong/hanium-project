...
**상태(2026-08-12)**: 이번 프로젝트 단계에서는 진행하지 않기로 확정(사용자 결정). 실제
사업자 정보(상호/대표자/사업자등록번호/주소/연락처, 개인정보 보호책임자 정보)가 없어
2026-08-06에 계속 보류로 시작했으나, 이후 별도 진행 계획 없이 이번 단계 범위에서 제외하기로
정리했다. placeholder와 초안 안내문은 화면에 그대로 유지된다(`frontend/src/pages/PrivacyPage.jsx`,
`TermsPage.jsx`). 값 주입 경로(`VITE_BUSINESS_*`, `VITE_PRIVACY_OFFICER_*` 환경변수,
`frontend/src/constants/businessInfo.js`)는 이미 준비돼 있으므로, 실제 서비스 공개를 다시
추진할 때는 코드 변경 없이 해당 값만 채우고 frontend를 재빌드하면 된다. **이 항목이 해결되지
않는 한 공개 출시는 불가하다** — 1장의 launch 차단 사유는 그대로 유효하다.
...
**상태(2026-08-12)**: 이번 프로젝트 단계에서는 진행하지 않기로 확정(사용자 결정). production
호스트가 없어 2026-08-06에 `deploy` job 자체를 계속 보류로 시작했으나(존재하지 않는 호스트를
향한 워크플로는 테스트 불가능한 죽은 코드가 된다), 이후 별도 진행 계획 없이 이번 단계 범위에서
제외하기로 정리했다. 호스트 결정과 무관한 하위 항목은 이미 처리돼 있다: SBOM 생성(CycloneDX,
...
Flyway 호환 리허설(P2-05 참고, 8.4 유지로 결정됨). keyless 서명·provenance 검증과 실제 deploy
job 구현은 production 호스트 운영을 다시 추진할 때 남은 작업이다.
...
**상태(2026-08-12)**: 이번 프로젝트 단계에서는 진행하지 않기로 확정(사용자 결정). 실제 SMTP
수신함, NVIDIA/OpenAI 유료 API 키, 운영 도메인, staging 호스트가 있어야 하는 항목들이라
2026-08-06에 계속 보류로 시작했으나, P1-06과 같은 이유(production 호스트 미정)로 이번 단계
범위에서 제외하기로 정리했다. 이 중 로컬에서 대체 가능했던 부분(MySQL 백업/복구 리허설,
