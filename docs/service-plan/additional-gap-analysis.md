...
## 업데이트: 2026-07-22 (같은 날 후속) video-llm-engine 서킷브레이커 실부하 검증 완료 — 해결

사용자 승인(비용 발생 사실을 먼저 알리고 명시적으로 진행 지시받음)을 받아 위 3번 항목을
실제 NVIDIA API로 검증했다. 사전에 그날 사용량(월간 1건 소진, 일일 한도 10건 중 여유 충분)을
확인한 뒤, VUS=5(전용 소규모 실행, 지속 부하 아님)로 실제 `VIDEO_LLM_ENABLED=true` 파이프라인을
1회 실행했다.

**결과**: 5건 모두 `COMPLETED`, 실패 0건. 이 중 **4건은 `generationMode=REAL`**(실제 NVIDIA
`nemotron-3-nano-omni` 호출 성공, 실비용 발생)이었고, **1건은 NVIDIA가 일시적으로 503을
반환해 자동으로 `FALLBACK`(mock)으로 전환**됐다 — 두 경로 모두 설계대로 정상 동작했다.
`video-llm-engine` 로그에서 개별 실제 호출의 실측 소요 시간은 **약 2.9~20초**로, 새로
설정한 slow-call 임계치(110초)에 큰 여유가 있음을 확인했다(참고로 k6가 보고한
`analysis_time_to_complete_seconds`, 즉 90~211초는 큐잉을 포함한 파이프라인 전체 시간이며
video-llm 호출 자체의 시간이 아니다 — 처음에는 이 둘을 혼동해 위협처럼 보였으나 로그 대조로
해소했다). 서킷브레이커 상태 전이/거부 이벤트는 backend·analysis-worker 로그 전 구간에서
0건이었다.

**비용**: 월간 카운터가 1→6(5 소진)으로 증가했고, 테스트 계정 5개 각각 일일 한도 10건 중
1건만 사용해 여유를 크게 남겼다. 테스트 계정은 전부 회원탈퇴로 정리했고 컨테이너는 원래
상태로 되돌렸다.

이로써 analysis-engine과 video-llm-engine 양쪽 모두 slow-call 임계치가 실측으로 검증됐다 —
이 항목은 완전히 해결된 것으로 정정한다.

### 다음 우선순위 재갱신 (2026-07-22, 같은 날 후속)

1. **staging 인프라 구축** — 유일하게 남은, 사용자의 실제 서버/클라우드 계정 결정이 필요한
   항목.
2. **Video LLM 실사용 관측을 계속 지켜보기** — 이전 업데이트와 동일하게 유효. 오늘 확인된
   NVIDIA 측 일시적 503(1/5건)은 폴백이 정상 동작했으므로 즉각 조치는 필요 없으나, 빈도가
   높아지면 `video_llm_generation_total{mode="FALLBACK"}` 알림으로 잡힐 것이다.

---

## 결정 기록 (2026-07-22): staging 인프라 구축은 진행하지 않기로 결정

위 1번 항목의 실제 구축 위치(국내 클라우드/글로벌 저가 VPS/AWS 등)와 예산을 사용자와
상의하는 과정에서, **이 프로젝트가 테스트베드(한이음 공모전) 성격임을 다시 고려해 staging
인프라 구축 자체를 진행하지 않기로 결정했다.** 실제 서버 비용을 들여 별도 staging 환경을
운영할 필요가 없다는 판단이다.

이 결정으로 "남은 staging 기록"(백필 규모 대조, 강제 중단 시나리오 등 로컬로 대체 불가능한
항목들, `docs/ops/minio-backfill-and-fallback-plan.md` 참고)과 "다음 우선순위" 1번 항목은
**의도적으로 보류**된 것으로 정정한다 — 미해결 갭이 아니라 프로젝트 성격에 맞춰 범위 밖으로
확정한 것이다. 이후 실제 서비스로 전환하는 시점이 오면(예: 한이음 심사 이후 실사용자 대상
운영을 고려하게 되면) 이 절을 갱신하고 재검토한다.

### 다음 우선순위 재갱신 (2026-07-22, 최종)

이번 세션에서 코드/실측 기준으로 진행 가능한 항목은 모두 소진됐다. 남은 항목은 전부
"프로젝트 범위 밖으로 의도적으로 보류"된 상태(staging 인프라)이거나 "지속 관찰"
성격(Video LLM 실사용 관측)이라, 별도의 즉시 착수 가능한 코드 작업은 남아 있지 않다.
다음 작업은 사용자가 새 방향(기능 추가, UI/UX 개선, 다른 컴포넌트 심화 검토 등)을 제시할
때 그에 맞춰 정하는 것을 권장한다.
