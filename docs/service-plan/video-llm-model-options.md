...
나머지만으로 부분 REAL 결과를 만들지 않는다. 모든 세그먼트 생성·NVIDIA 호출이
성공해야 REAL로 합치며, 하나라도 실패하면 전체 호출을 실패시켜 STRICT/requireReal은
실패하고 DEGRADED 일반 분석만 기존 `FALLBACK` mock 경로로 넘어간다.
...
**2026-07-27 비용 가드 보완**: backend의 `video-llm-monthly` 카운터는 더 이상 작업
1건을 무조건 1회로 세지 않는다. 영상 길이와 video-llm-engine과 공유하는
`VIDEO_LLM_CHUNK_DURATION_SECONDS`로 예상 세그먼트 수를 계산해 Lua로 월간 용량 안에서
원자적으로 예약한다(예: 30분/100초는 18회). 길이 재확인이 실패하면 업로드 최대 길이
기준으로 보수적으로 예약한다. `video-llm-daily`는 사용자별 분석 작업 수를 제한하는
목적이므로 기존처럼 작업당 1회를 유지한다. 실제 공급자 계약 한도 자체는 여전히
확인되지 않았으므로 capacity 값의 운영 확정은 별도 과제다.
