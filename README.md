...
업로드 화면은 `GET /analyze/practice/purposes`에서 발표 목적, 권장 시간, 평가 초점을
조회한다. 분석 결과에는 다음 연습 행동 3가지, 예상 질문, 발화 시점 기반 내용 구조
보조 분석, 분석 신뢰도를 표시한다. 연습 코칭 요청이 실패해도 기본 분석 결과와
타임라인은 독립적으로 표시된다.

감지할 수 없거나 데이터가 부족한 자세·얼굴 방향 지표는 `0점`이 아닌 `측정 불가`로
표시하고 종합 점수 계산에서 제외한다. 얼굴 방향 안정성은 카메라 기준 얼굴 방향
추정이며 실제 시선 추적 결과가 아니다.

새 분석 알고리즘 `2026.06.2`부터 자세·얼굴 감지율은 발표 실력 점수가 아닌 분석
신뢰도로만 사용한다. 시각 평가는 최소 3개 유효 프레임과 30% 감지율을 충족해야
점수로 확정한다. 한국어 말하기 속도는 음절/분을 우선 사용하며, 필러와 손동작
...
`docs/PRACTICE_COACHING_OPERATIONS.md`에서 확인한다.

결과 상세 화면의 `AI 발표 코칭`은 선택 기능이다. 생성 요청 시 기존 정량 점수를
...
`./scripts/verify.sh`는 `DISABLE_BACKGROUND_SERVICES=true`로 백엔드 단위 테스트를
실행하여 실제 DB 복구 작업과 분석 워커를 시작하지 않는다.
...
`docs/PRODUCTION_CHECKLIST.md`, 연습 코칭 운영 기준은
`docs/PRACTICE_COACHING_OPERATIONS.md`에서 확인한다.
