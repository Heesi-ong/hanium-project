...
## 13. 2026-07-31 원본/재분석 직접 비교 UX

일반 결과 목록에는 완료 결과 2개를 임의 선택하는 비교 기능과
`/results/compare` 페이지가 이미 있었다. 그러나 상세 페이지의 lineage 카드는 원본과
최신 재분석 상세로 이동하는 링크만 제공해, 사용자가 목록으로 돌아가 두 job을 다시
찾아 선택해야 했다.

상세 lineage 카드에 `원본과 재분석 비교`를 추가했다. 클릭하면 현재 로그인 사용자의
소유권 검증이 적용되는 기존 `GET /api/results/{jobId}`로 상대 결과를 조회하고,
두 결과가 모두 `COMPLETED`일 때 원본을 왼쪽, 재분석을 오른쪽으로 고정해 기존 비교
페이지로 이동한다. 상대 작업이 아직 완료되지 않았거나 조회에 실패하면 상세 화면에
오류를 표시하며, 별도 백엔드 API나 우회 조회는 추가하지 않았다.

검증 결과:

- ResultDetailPage 대상 테스트 8 passed
