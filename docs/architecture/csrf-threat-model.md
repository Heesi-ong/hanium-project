...
| `POST /api/auth/login` | `AuthController.login` | `@RequestBody AuthRequest`; intended JSON | Plain form is not valid JSON. In local curl, no-Origin form-urlencoded produced a 500 error; Origin-bearing form was rejected by CORS. | No (`permitAll`) | Preflight/simple request from unallowed origin rejected | JSON login CSRF is blocked by preflight. The 500 on wrong content type is an error-handling issue, not a CSRF bypass. |
